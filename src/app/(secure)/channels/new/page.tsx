'use client'

import { useEffect, useMemo, useState } from 'react'
import { SidebarTrigger } from '@/components/ui/sidebar'
import { Card, CardContent, CardHeader, CardTitle } from '@/components/ui/card'
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from '@/components/ui/select'
import { Input } from '@/components/ui/input'
import { Label } from '@/components/ui/label'
import { Button } from '@/components/ui/button'
import { toast } from '@/components/ui/sonner'
import { useSearchParams } from 'next/navigation'

type Provider = 'telegram' | 'discord' | 'sms' | 'whatsapp' | 'email' | 'web'

export default function NewChannelWizardPage() {
  const params = useSearchParams()
  const preset = (params.get('provider') as Provider) || undefined

  const [provider, setProvider] = useState<Provider | ''>(preset || '')
  const [step, setStep] = useState<number>(1)

  // Collected values (progressively revealed)
  const [botToken, setBotToken] = useState('')
  const [emailProvider, setEmailProvider] = useState<'sendgrid'|'smtp'|''>('')
  const [twilioSid, setTwilioSid] = useState('')
  const [twilioAuth, setTwilioAuth] = useState('')

  useEffect(()=>{
    if (preset) {
      setProvider(preset)
    }
  },[preset])

  const canContinue = useMemo(()=>{
    if (step === 1) return provider !== ''
    if (step === 2) {
      switch(provider) {
        case 'telegram': return botToken.length > 0
        case 'email': return emailProvider !== ''
        case 'sms': return twilioSid.length > 0 && twilioAuth.length > 0
        default: return true
      }
    }
    return true
  }, [step, provider, botToken, emailProvider, twilioSid, twilioAuth])

  const next = async () => {
    if (!canContinue) return
    // Contextual flow
    if (step === 1) {
      setStep(2)
      return
    }
    if (step === 2) {
      try {
        if (provider === 'telegram') {
          const res = await fetch('/api/channels/telegram/connect', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ botToken }) })
          const data = await res.json()
          if (!res.ok || !data.status) throw new Error(data.message || 'Failed')
          toast.success('Telegram connected')
        } else if (provider === 'email') {
          toast.success('Email channel saved (stub)')
        } else if (provider === 'sms') {
          toast.success('SMS channel saved (stub)')
        }
        setStep(3)
      } catch (e:any) {
        toast.error(e.message || 'Failed to save settings')
      }
      return
    }
  }

  return (
    <div className="flex-1 p-2 w-full bg-background">
      <div className="mx-auto rounded-lg bg-card text-card-foreground shadow-lg">
        <div className="px-6 py-2">
          <div className="flex items-center">
            <SidebarTrigger className="-ml-0.5 h-5 w-5 text-muted-foreground hover:text-foreground" />
            <div className="h-4 border-l border-border mx-4" />
            <h1 className="text-base font-medium text-card-foreground">Create New Channel</h1>
          </div>
        </div>
        <div className="border-b border-border" />

        <div className="p-6 max-w-2xl space-y-4">
          <Card>
            <CardHeader>
              <CardTitle>{step}. {step === 1 ? 'Basic Information' : step === 2 ? 'Channel Configuration' : 'Complete Setup'}</CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              {step === 1 && (
                <div className="grid gap-4">
                  <div className="grid gap-2">
                    <Label>Channel Type</Label>
                    <Select value={provider} onValueChange={(v)=> setProvider(v as Provider)}>
                      <SelectTrigger><SelectValue placeholder="Select a type" /></SelectTrigger>
                      <SelectContent>
                        <SelectItem value="telegram">Telegram</SelectItem>
                        <SelectItem value="discord">Discord</SelectItem>
                        <SelectItem value="sms">SMS</SelectItem>
                        <SelectItem value="whatsapp">WhatsApp</SelectItem>
                        <SelectItem value="email">Email</SelectItem>
                        <SelectItem value="web">Web (built-in)</SelectItem>
                      </SelectContent>
                    </Select>
                    <p className="text-xs text-muted-foreground">Pick a channel. The next step will adapt to your choice.</p>
                  </div>
                  <div>
                    <Button onClick={next} disabled={!canContinue}>Continue</Button>
                  </div>
                </div>
              )}

              {step === 2 && provider === 'telegram' && (
                <div className="grid gap-4">
                  <div className="space-y-2">
                    <Label>Bot Token</Label>
                    <Input type="password" value={botToken} onChange={e=>setBotToken(e.target.value)} placeholder="123456:ABC-DEF1234..." />
                    <p className="text-xs text-muted-foreground">Create a bot with @BotFather → /newbot → copy token.</p>
                  </div>
                  <div className="rounded-md border p-3 text-xs bg-muted">
                    After connecting, your webhook will be configured automatically and you can enable Telegram per survey.
                  </div>
                  <div>
                    <Button onClick={next} disabled={!canContinue}>Connect</Button>
                  </div>
                </div>
              )}

              {step === 2 && provider === 'email' && (
                <div className="grid gap-4">
                  <div className="space-y-2">
                    <Label>Email Provider</Label>
                    <Select value={emailProvider} onValueChange={(v)=> setEmailProvider(v as any)}>
                      <SelectTrigger><SelectValue placeholder="Select" /></SelectTrigger>
                      <SelectContent>
                        <SelectItem value="sendgrid">SendGrid</SelectItem>
                        <SelectItem value="smtp">SMTP</SelectItem>
                      </SelectContent>
                    </Select>
                  </div>
                  <div className="rounded-md border p-3 text-xs bg-muted">We’ll ask for the right credentials based on your pick in the next step.</div>
                  <div>
                    <Button onClick={next} disabled={!canContinue}>Continue</Button>
                  </div>
                </div>
              )}

              {step === 2 && provider === 'sms' && (
                <div className="grid gap-4">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    <div className="space-y-2">
                      <Label>Twilio SID</Label>
                      <Input value={twilioSid} onChange={e=>setTwilioSid(e.target.value)} />
                    </div>
                    <div className="space-y-2">
                      <Label>Twilio Auth Token</Label>
                      <Input type="password" value={twilioAuth} onChange={e=>setTwilioAuth(e.target.value)} />
                    </div>
                  </div>
                  <div className="rounded-md border p-3 text-xs bg-muted">We’ll verify credentials and set a webhook in the next step.</div>
                  <div>
                    <Button onClick={next} disabled={!canContinue}>Continue</Button>
                  </div>
                </div>
              )}

              {step === 3 && (
                <div className="space-y-3 text-sm">
                  <p>All set. Go to a survey’s Channels tab to enable this channel and get the per-survey deep link/QR.</p>
                </div>
              )}
            </CardContent>
          </Card>
        </div>
      </div>
    </div>
  )
}


