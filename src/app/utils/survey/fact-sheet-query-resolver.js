class FactSheetQueryResolver {
  ...
  resolveFromFactSheet(question, factSheet) {
    ...
    const platformStats = Object.values(questionStats).find((stats) => 
    ...
    const usageStats = Object.values(questionStats).find((stats) => 
  ...
  resolvePlatformQuestion(question, platformStats, factSheet) {
    ...
    if (question.includes('most popular') || question.includes('top platform') || question.includes('most used')) {
      ...
      if (sortedPlatforms.length > 1) {
        ...
        answer += ` The top 3 platforms are: ${top3.map(([name, stats]) => `${name} (${stats.percentage}%)`).join(', ')}.`;
      }
    ...
    // Platform combination questions
      ...
      const top3 = sortedPlatforms.slice(0, 3);
      answer = `Based on platform adoption rates, the most commonly used platforms are: ${top3.map(([name, stats]) => `${name} (${stats.percentage}%)`).join(', ')}. `;
    ...
    // Platform comparison questions
      ...
      answer = `Platform adoption rates: ${sortedPlatforms.map(([name, stats]) => `${name}: ${stats.percentage}% (${stats.users} users)`).join(', ')}.`;
      confidence = 0.9;
    }
    // General platform usage questions
    else {
      answer = `Platform adoption breakdown: ${sortedPlatforms.map(([name, stats]) => `${name} is used by ${stats.percentage}% of respondents (${stats.users} users)`).join(', ')}.`;
      confidence = 0.85;
    }
    ...
    dataCards.push({
      ...
      data: sortedPlatforms.slice(0, 5).map(([platform, stats]) => ({
        label: platform,
  ...
  resolveUsageQuestion(question, usageStats, factSheet) {
  ...
  resolveDemographicQuestion(question, demographics, factSheet) {
    let answer = "";
    let dataCards = [];
    let confidence = 0.8;
    ...
    if (question.includes('age') && demographics.age) {
      ...
      answer = `Age distribution: ${ageData.map(([group, stats]) => `${group}: ${stats.percentage}% (${stats.count} respondents)`).join(', ')}.`;
      
      dataCards.push({
        type: 'age_distribution',
        title: 'Age Distribution',
        data: ageData.map(([group, stats]) => ({
          label: group,
          value: stats.percentage,
          count: stats.count
        })),
      ...
      answer = `Gender distribution: ${genderData.map(([group, stats]) => `${group}: ${stats.percentage}% (${stats.count} respondents)`).join(', ')}.`;
      confidence = 0.9;
    } else {
      ...
      if (demographics.age) {
        const topAge = Object.entries(demographics.age)[0];
  ...
  resolveOverviewQuestion(question, factSheet, questionStats) {
    ...
    if (platformStats) {
      const topPlatform = Object.entries(platformStats.adoption_rates)[0];
...
module.exports = { FactSheetQueryResolver }; 