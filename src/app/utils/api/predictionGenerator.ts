...
export class AIEnhancedPredictionGenerator {
    ...
    private calculateStakeBasedOnConfidence(confidence: number): number {
        if (confidence >= 0.8) {
            return Math.min(this.agent.aggressiveBetSize, this.agent.maxBetSize);
        } else if (confidence >= 0.6) {
            return Math.min(this.agent.moderateBetSize, this.agent.maxBetSize);
        } else {
            return Math.min(this.agent.conservativeBetSize, this.agent.maxBetSize);
        }
    }
    ...
    async generatePrediction(): Promise<AutomatedPrediction | undefined> {
        ...
        prediction.principlesApplied = this.agent.principles;
