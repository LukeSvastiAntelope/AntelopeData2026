...
export async function POST(request: NextRequest) {
  try {
    ...
    const {
      ...
      office_type, state, district_code, candidate_name, party, election_year,
    } = await request.json();
    ...
    // Create the organization with optional location and campaign fields
    const [result]: any = await db.execute(
      `INSERT INTO organizations
        (name, slug, description, created_by, latitude, longitude, default_zoom,
         office_type, state, district_code, candidate_name, party, election_year)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        name.trim(),
