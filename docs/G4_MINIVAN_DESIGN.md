...
## Explicitly built (MiniVAN M2–M4)
...
- Live progress polling (`GET /api/dashboard/canvass-progress`) — coverage %, outcome breakdown, last-synced on Turf + Assignments (~5s)

## Still later

- Full websocket / SSE real-time layer (polling is enough for managers watching doors land)
